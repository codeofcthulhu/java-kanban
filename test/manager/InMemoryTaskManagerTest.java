...
class InMemoryTaskManagerTest {
    ...
    void taskInHistoryShouldHasOldData() {
        ...
        Assertions.assertFalse(task0.getName().equals(taskFromHistory.getName()));
        Assertions.assertFalse(task0.getDescription().equals(taskFromHistory.getDescription()));
        Assertions.assertFalse(task0.getStatus() == taskFromHistory.getStatus());
        Assertions.assertFalse(task0.getId() == taskFromHistory.getId());
    ...
    // Конец тестов из ТЗ
    // Тесты по методам класса InMemoryTaskManager
    @Test
    ...
    @Test
    void shouldReturnHistoryWithTenTasks() {
        Task task0 = new Task("Заголовок первого таска", "Описание первого таска", Status.NEW);
        taskManager.createTask(task0);
